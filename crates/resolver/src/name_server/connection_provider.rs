...
impl<P: RuntimeProvider> ConnectionProvider for GenericConnector<P> {
    ...
    fn new_connection(
        ...
        let dns_connect = match (config.protocol, self.runtime_provider.quic_binder()) {
            ...
            (Protocol::Mdns, _) => {
                let (connect, stream) =
                    MdnsClientStream::new_ipv4(MdnsQueryType::OneShotJoin, Some(32), None);
                let connect = DnsMultiplexer::new(connect, stream, None);

                let exchange = DnsExchange::connect::<_, _, TokioTime>(connect);
